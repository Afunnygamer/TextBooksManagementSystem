...
void Widget::on_Import_clicked()
{
    ...
    QString path = QFileDialog::getOpenFileName(this,"open","../","execl(*.xls)");
...
void Widget::on_Export_clicked()
{
    ...
    ui->progressBar->setValue(0);   //设置进度条的值为0
    QString fileName = QFileDialog::getSaveFileName(this,tr("Excle file"),QString("./Excel.xls"),tr("Excel Files(*.xls)"));    //设置保存的文件名
    qDebug() << fileName;
    if(fileName != "")
    {
        ui->progressBar->setValue(10);
